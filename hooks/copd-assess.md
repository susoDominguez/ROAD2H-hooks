...
<mark>The `copd-assess` hook is triggered when the pulmonologist has completed the airflow limitation severity assessment on the current COPD patient, to adjust medication if needed. The assessment is completed when the final values has been stored in the patient's record for the COPD assessment test (CAT) score and Medical Research Council (mMRC) Dyspnoea scale score.</mark>

## Context
<mark>The context contains pairs of measurements of CAT score and mMRC dyspnoea scale, one pair taken on the current encounter and another (possibly none if first visit) as stored in the previous COPD follow-up. Similarly for the number of COPD exacerbations. Additionally, the context contains information on whether an asthma diagnosis is present on the patient as well as information on the currently active COPD treatment, that is, both the personalised COPD group and its associated active medication.</mark>
...
<mark>`previousAssessment`</mark> | OPTIONAL | No | *object* | <mark>FHIR Bundle of Observations in 'final' state representing personalised COPD group, CAT score, mMRC dyspnoea scale and number of exacerbations as recorded on the previous COPD-related encounter. Omission of the bundle resource entirely suggests patient has just been newly diagnosed with COPD at this encounter.</mark>
...
<mark>`asthma`</mark> | OPTIONAL | No | *object* | <mark>Condition or Observation resource denoting the presence of an asthma diagnosis, or history of asthma, in the patient's record.</mark>
